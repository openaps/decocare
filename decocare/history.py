...
class LowReservoir(KnownRecord):
    ...
    opcode = 0x34
    _test_1 = bytearray([0x34, 0xC8, 0xEB, 0x02, 0x0B, 0x07, 0x0C,])
...
class AlarmSensor(KnownRecord):
    ...
    def decode(self):
        ...
        decoded_dict = {
            ...
            "alarm_description": self.alarm_types.get(
                alarm_type, "Unknown sensor alarm ({})".format(alarm_type)
            ),
        }
...
def decode_insulin_sensitivies(data):
    ...
    for x in range(8):
        ...
        sensitivities.append(
            dict(i=x, offset=offset * 30, _offset=offset, sensitivity=sensitivity)
        )
