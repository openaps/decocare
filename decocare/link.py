...
class Link:
    ...
    def open(self, newPort=False, **kwds):
        ...
        if self.serial.isOpen():
            log.info(
                "{agent} opened serial port: {serial}".format(
                    serial=repr(self.serial), agent=self.__class__.__name__
                )
            )
    ...
    def write(self, string):
        ...
        io.info(
            "usb.write.len: {}\n{}".format(len(string), lib.hexdump(bytearray(string)))
        )
        return r
    ...
    def readline(self):
        ...
        io.info("usb.read.len: {}\n{}".format(len(r), lib.hexdump(bytearray(r))))
        return r

    def readlines(self):
        r = self.serial.readlines()
        io.info(
            "usb.read.len: {}\n{}".format(len(r), lib.hexdump(bytearray("".join(r))))
        )
        return r
