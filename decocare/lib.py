...
def _fmt_txt(bytez):
    return "".join([chr(x) if 0x20 <= x < 0x7F else "." for x in bytez])
...
def format_filter_date(date):
    ...
    [7, 222, 4, 9]
    """
    params = [HighByte(date.year), LowByte(date.year), date.month, date.day]
    return params
...
def hexdump(src, length=8, indent=0):
  ...
  line.
    ...
    for i in range(0, len(src), length):
        ...
        result.append(indent + "%04X   %-*s   %s" % (i, length * 5, hexa, text))
    return "\n".join(result)
...
_enc_result_2 = [0xA9, 0x6D, 0x16, 0x8E, 0x39, 0xB2, 0x68, 0xD5, 0x55, 0xAA, 0x65]
...
_decode_result_1 = [0xA7, 0x47, 0x33, 0x62, 0x8D, 0x09, 0x03, 0x37, 0x32, 0x32,
                    ...
                    0xC2]
# fmt:on
_decode_test_2 = [0xA9, 0x6D, 0x16, 0x8E, 0x39, 0xB2, 0x56, 0x65, 0x55, 0x56, 0x35]
