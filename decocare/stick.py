...
class ProductInfo(StickCommand):
  ...
  Eg, European vs US regulatory domains require different frequencies for compliance.
    ...
    rf_table = {1: "868.35Mhz", 0: "916.5Mhz", 255: "916.5Mhz"}
  ...
  the current state of the radio/stick.
    ...
    def __str__(self):
        ...
        if getattr(self, "error", False):
            extra += "{}:error:{}:reason:{}".format(
                self.__class__.__name__, self.error, str(self.reasons)
            )
  ...
  Downloads a packet from the radio buffer.
    ...
    def __init__(self, size):
        ...
        if size < 64 and size != 15:
            log.error(
                "size (%s) is less than 64 and not 15, which may cause an error." % size
            )
            self.size = 64
    ...
    def respond(self, raw):
        ...
        log.info(
            "readData; foreign raw should be at least 14 bytes? {} {}".format(
                len(raw), len(raw) > 14
            )
        )
    ...
    """
        ...
        log.info(
            "found packet len({}), link expects({})".format(len(self.packet), resLength)
        )
...
class Stick:
  ...
  * download - loop
  ...
  will represent the state of one active usb stick.
    ...
    """
        msg = ":".join(
            ["PROCESS", "START"] + list(map(str, [self.timer.millis(), self.command]))
        )
        log.info(msg)
    ...
    """
        ...
        msg = ":".join(
            ["PROCESS", "END"] + list(map(str, [self.timer.millis(), self.command]))
        )
        log.info(msg)
        return info
    ...
    """
        ...
        try:
            ...
            if original_size < 64:
                ...
                # size = self.poll_size( )
                log.info(
                    "XXX:JUST a bit more READ new size: %s, sleep .100" % original_size
                )
    ...
    """
        ...
        while not eod:
            ...
            if size is None:
                log.info(
                    "%s:begin first poll first sleep .250"
        ...
        for attempt in range(3):
            ...
            while size > 14:
                ...
                log.info(
                    ":".join(
                        [
                            "{} downloaded {} segment".format(log_head, len(raw)),
                            log_detail,
                            "RAW:\n%s" % lib.hexdump(raw),
                        ]
                    )
