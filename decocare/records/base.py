    ...
    def pformat(self, prefix=""):
        head = "\n".join(
            [
                ...
                lib.hexdump(self.head, indent=4),
                "    decimal",
                lib.int_dump(self.head, indent=11),
            ]
        )
        date = "\n".join(
            ["    datetime (%s)" % self.date_str(), lib.hexdump(self.date, indent=4)]
        )
        ...
        if len(self.body) > 0:
            body = "\n".join(
                [
                    body,
                    "    hex",
                    lib.hexdump(self.body, indent=4),
                    "    decimal",
                    lib.int_dump(self.body, indent=11),
                ]
            )
        extra = []
        """
