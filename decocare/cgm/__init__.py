    ...
    """
        ...
        for B in iter(lambda: self.stream.read(1), ""):
            ...
            if record["name"] == "DataEnd":
                ...
                record["name"] == "GlucoseSensorData"
                or record["name"] == "SensorWeakSignal"
                or record["name"] == "SensorCal"
                or record["name"] == "19-Something"
            ):
                ...
                record["name"] == "SensorTimestamp"
                or record["name"] == "SensorCalFactor"
                or record["name"] in ["10-Something"]
            ):
                ...
                mapped_glucose_records = self.map_glucose(
                    prefix_records, start=date, delta=self.delta_ago(reverse=True)
                )
            ...
            """
                    if record["name"] == "CalBGForGH":
                        amount = lib.BangInt(
                            [(raw_packet[2] & 0b00100000) >> 5, body[0]]
                        )
                        record.update(body=self.byte_to_str(body))
