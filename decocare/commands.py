...
class PumpCommand(BaseCommand):
    ...
    def __str__(self):
        if self.responded:
            return "{}:size[{}]:data:{}".format(
                self.__class__.__name__, self.size, repr(self.getData())
            )
    ...
    def format(self):
        ...
        packet = head + serial
        # paramCount 2 bytes
        packet.extend([(0x80 | lib.HighByte(paramsCount)), lib.LowByte(paramsCount)])
...
class TempBasal(PumpCommand):
    ...
    def Program(klass, rate=None, duration=None, temp=None, **kwds):
        assert duration % 30 == 0, "duration {} is not a whole multiple of 30".format(
            duration
        )
        assert temp in [
            "percent",
            "absolute",
        ], "temp field <{}> should be one of {}".format(temp, ["percent", "absolute"])
        if temp in ["percent"]:
            return TempBasalPercent(
                params=klass.format_percent_params(rate, duration), **kwds
            )
...
class ReadHistoryData(PumpCommand):
    ...
    def done(self):
        ...
        explain_crc = "CRC ACK check found[{}] expected[{}]".format(
            found_crc, expect_crc
        )
        is_eod = "and has eod set? %s" % (eod)
        log.info(
            "{}:download:done {}:{}:{}".format(self, expect_size, explain_crc, is_eod)
        )
        return found >= expect

    def respond(self, raw):
        log.info(
            "{} extending original {} with found {}".format(
                str(self), len(self.data), len(raw)
            )
        )
        if len(raw) == self.size:
            log.info(
                "{} download respond replace original {} with found {}".format(
                    str(self), len(self.data), len(raw)
                )
            )
            self.data = raw
            ...
            log.info(
                "{} download respond original {}, XXX IGNORE found {}".format(
                    str(self), len(self.data), len(raw)
                )
            )
            pass
            ...
            log.info(
                "{} download respond extend original {} with found {}".format(
                    str(self), len(self.data), len(raw)
                )
            )
...
class ReadRTC(PumpCommand):
    ...
    def getData(self):
        ...
        return "{year:#04}-{month:#02}-{day:#02}T{hour:#02}:{minute:#02}:{second:#02}".format(
            **d
        )
...
class ReadCarbRatios512(PumpCommand):
    ...
    def getData(self):
        ...
        return dict(
            schedule=self.decode_ratios(data[0:], units=units),
            units=labels.get(units),
            first=self.data[0],
            raw=" ".join("0x{:02x}".format(x) for x in self.data),
        )
...
class ReadCarbRatios(PumpCommand):
    ...
    def getData(self):
        ...
        return dict(
            schedule=self.decode_ratios(data, units=units),
            units=labels.get(units),
            first=self.data[0],
        )
...
""".strip()
    ...
    resp_high_bit_broken = {
        "units": "mg/dL",
        "sensitivities": [
            {"i": 64, "start": "08:00:00", "sensitivity": 12, "offset": 1920, "x": 0}
        ],
        "first": 1,
    }
    resp_high_bit_fixed = {
        "units": "mg/dL",
        "sensitivities": [
            {"i": 0, "start": "00:00:00", "sensitivity": 268, "offset": 0, "x": 0}
        ],
        "first": 1,
    }
    ...
    def decode(data):
        ...
        return dict(
            ...
            first=units,
            units=ReadInsulinSensitivities.UNITS.get(units),
        )
...
class ReadProfile_STD512(PumpCommand):
    ...
    _test_result_1 = bytearray([32, 0, 0, 38, 0, 13, 44, 0, 19, 38, 0, 28] + [0] * 52)
    _test_schedule = {
        "total": 22.50,
        "schedule": [
    ...
    def decode(data):
        ...
        for i in range(len(data) // 3):
            ...
            if i > 0 and [r, z, m] in [end, none]:
                break
            schedule.append(
                dict(i=i, minutes=m * 30, start=str(lib.basal_time(m)), rate=r * 0.025)
            )
        return schedule
...
class ReadPumpStatus(PumpCommand):
    ...
    def getData(self):
        ...
        normal = {0o3: "normal"}
        status = {
            "status": normal.get(data[0], "error"),
            "bolusing": data[1] == 1,
            "suspended": data[2] == 1,
        }
        return status
...
class TweakAnotherCommand(ManualCommand):
    ...
    def config_argparse(klass, parser):
        ...
        parser.add_argument(
            "--page", type=int, help="Page to fetch (for ReadHistoryData)"
        )

        parser.add_argument(
            "--begin", type=lib.parse.date, help="begin date for FilterHistory"
        )
        parser.add_argument(
            "--end", type=lib.parse.date, help="end date for FilterHistory"
        )
...
if __name__ == "__main__":
    ...
    stick = stick.Stick(link.Link(port, timeout=0.400))
    stick.open()
