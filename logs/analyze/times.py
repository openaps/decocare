...
def encode_monthbyte(sec=18, minute=30, month=10):
    """
    >>> encode_monthbyte( ) == bytearray(bytes('\x92\x9e', 'raw_unicode_escape'))
    True

    >>> quick_hex(encode_monthbyte( ))
    '0x92 0x9e'

    >>> encode_monthbyte(sec=10) == bytearray(bytes('\x8a\x9e', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(sec=35) == bytearray(bytes('\xa3\x9e', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(sec=50) == bytearray(bytes('\xb2\x9e', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(minute=10) == bytearray(bytes('\x92\x8a', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(minute=35) == bytearray(bytes('\x92\xa3', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(minute=50) == bytearray(bytes('\x92\xb2', 'raw_unicode_escape'))
    True
...
def encode_second(sec=18, month=10):
    """
    >>> encode_second( ) == bytearray(bytes('\x92', 'raw_unicode_escape'))
    True
...
__test__ = {
    ...
    """,
    "encode_monthbyte": """
    >>> encode_monthbyte(month=1) == bytearray(bytes('\x12^', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(month=2) == bytearray(bytes('\x12\x9e', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(month=3) ==  bytearray(bytes('\x12\xde', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(month=10, minute=0, sec=0) == bytearray(bytes('\x80\x80', 'raw_unicode_escape'))
    True

    >>> encode_monthbyte(month=10, minute=0, sec=24) == bytearray(bytes('\x98\x80', 'raw_unicode_escape'))
    True
    """,
}
