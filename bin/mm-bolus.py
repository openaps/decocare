...
class BolusApp(cli.CommandApp):
    ...
    def customize_parser(self, parser):
        ...
        group.add_argument(
            "--515", dest="strokes_per_unit", action="store_const", const=10
        )
        group.add_argument(
            "--554", dest="strokes_per_unit", action="store_const", const=40
        )
    ...
    def bolus(self, args):
        ...
        resp = self.exec_request(
            self.pump, query, args=kwds, dryrun=args.dryrun, render_hexdump=False
        )
        return resp
