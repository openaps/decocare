...
from decocare import cgm, commands, models
...
class DownloadHistory(cli.CommandApp):
    ...
    def customize_parser(self, parser):
        ...
        parser.add_argument(
            "--timezone", default=gettz(), type=gettz, help="Timezone to use"
        )
        parser.add_argument(
            "--parser-out",
            dest="parsed_data",
    ...
    def download_page(self, number):
        return self.exec_request(
            self.pump,
            commands.ReadGlucoseHistory,
            args=dict(page=number),
    ...
    def download_history(self, args, pageRange):
        ...
        for i in range(1 + pageRange["page"] - pageRange["isig"], pageRange["page"]):
            ...
            try:
                ...
                print(
                    "Unexpected error when downloading cgm-page ",
                    i,
                    " from pump:",
                    sys.exc_info()[0],
                )
    ...
    def getPagesRange(self):
        range = self.exec_request(
            self.pump,
            commands.ReadCurGlucosePageNumber,
            render_decoded=False,
            render_hexdump=False,
        )
